...
void AnimationsCtr::setupDemos(){
	...
	xSpeedPicker.setColour(xColour);
	xSpeedPicker.setStrip(pStrip, xLength);
	xSpeedPicker.setStepMS(xStepMS);
}
...
/***
 * change the speed
 * @param inc - true then increase speed, false decrease
 */
void AnimationsCtr::changeSpeed(bool inc){
	if (inc){
		xSpeedPicker.incrementSpeed(SPEED_STEP);
	} else {
		xSpeedPicker.derementSpeed(SPEED_STEP);
	}
}
...
void AnimationsCtr::handleLongPress(uint8_t gp){
	if (xMode == AnimationsDemos){
		xMode = AnimationsSetSpeed;
		xSpeedPicker.setColour(xColour);
	} else if (xMode == AnimationsSetSpeed){
		xStepMS = xSpeedPicker.getStepMS();
		pDemos[xDemo]->setStepMS(xStepMS);
		xMode = AnimationsDemos;
	}
}
...
/***
 * Switch LED power on or off
 * @param on - true if on
 */
void AnimationsCtr::switchPower(bool on){
	xPower = on;
	if (!xPower){
		pStrip->clear();
		pStrip->show();
